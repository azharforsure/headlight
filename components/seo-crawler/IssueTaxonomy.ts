export const SEO_ISSUES_TAXONOMY = [
    {
        ...
        issues: [
            ...
            { id: 'sitemap_broken_urls', checkId: 't1-sitemap-validity', label: 'Broken URLs Found In Sitemap', type: 'warning', condition: (p: any) => Number(p.crawlDepth || 0) === 0 && Number(p.sitemapBrokenUrls || 0) > 0 },
            { id: 'sitemap_stale_lastmod', checkId: 't1-sitemap-lastmod', label: 'Sitemap Lastmod Mismatch', type: 'notice', condition: (p: any) => p.sitemapLastmodAccurate === false || (Number(p.crawlDepth || 0) === 0 && Number(p.sitemapLastmodMismatchCount || 0) > 0) },
    ...
    },
    {
        ...
        issues: [
            ...
            { id: 'ssl_grade_low', checkId: 't1-ssl-grade', label: 'SSL Grade Below A', type: 'warning', condition: (p: any) => p.url?.startsWith('https://') && p.sslGrade && !['A', 'A+'].includes(p.sslGrade) },
            { id: 'ssl_chain_invalid', checkId: 't1-ssl-chain', label: 'Incomplete SSL Chain', type: 'warning', condition: (p: any) => p.url?.startsWith('https://') && p.sslChainComplete === false },
            { id: 'cname_chain_long', checkId: 't1-dns-cname-chain', label: 'Long CNAME Chain (3+)', type: 'notice', condition: (p: any) => Number(p.cnameChainLength || 0) >= 3 },
            ...
            { id: 'hydration_mismatch', checkId: 't1-js-hydration', label: 'Hydration Mismatch', type: 'warning', condition: (p: any) => p.hydrationMismatch === true },
    ...
    },
    {
        ...
        issues: [
            ...
            { id: 'table_no_headers', checkId: 't2-a11y-table', label: 'Table Missing Headers', type: 'notice', condition: (p: any) => p.isHtmlPage && Number(p.tablesWithoutHeaders || 0) > 0 },
    ...
    },
    {
        ...
        issues: [
            ...
            { id: 'missing_local_schema', checkId: 't4-local-schema', label: 'Missing LocalBusiness Schema', type: 'warning', condition: (p: any) => p.industry === 'local' && !p.industrySignals?.hasLocalBusinessSchema },
    ...
    },
    {
        ...
        issues: [
            { id: 'low_content_quality', checkId: 't3-content-quality', label: 'Low AI Content Quality (<40)', type: 'warning', condition: (p: any) => p.isHtmlPage && Number(p.contentQualityScore) > 0 && Number(p.contentQualityScore) < 40 },
            { id: 'intent_mismatch', checkId: 't3-content-intent', label: 'Search Intent Mismatch', type: 'warning', condition: (p: any) => p.isHtmlPage && p.intentMismatch === true },
            { id: 'low_eeat', checkId: 't3-content-eeat', label: 'Low E-E-A-T Score (<40)', type: 'warning', condition: (p: any) => p.isHtmlPage && Number(p.eeatScore) > 0 && Number(p.eeatScore) < 40 },
            { id: 'negative_sentiment', checkId: 't3-content-sentiment', label: 'Negative Content Sentiment', type: 'notice', condition: (p: any) => p.isHtmlPage && String(p.sentiment || '').toLowerCase() === 'negative' },
            { id: 'low_originality', checkId: 't3-content-originality', label: 'Low Originality Score (<30)', type: 'notice', condition: (p: any) => p.isHtmlPage && Number(p.originalityScore) >= 0 && Number(p.originalityScore) < 30 },
            { id: 'likely_ai_generated', checkId: 't3-ai-generated', label: 'Likely AI-Generated Content', type: 'notice', condition: (p: any) => p.isHtmlPage && ['high', 'very_high'].includes(String(p.aiLikelihood || '').toLowerCase()) },
    ...
    },
    {
        ...
        issues: [
            ...
            { id: 'spa_routing_broken', checkId: 't1-spa-routing', label: 'SPA Route Not Crawlable Without JS', type: 'warning', condition: (p: any) => p.spaRouteBroken === true },
...
export const ISSUE_TO_CHECK_MAP: Record<string, string> = {
    ...
    sitemap_broken_urls: 't1-sitemap-validity',
    sitemap_stale_lastmod: 't1-sitemap-lastmod',
    ...
    low_content_quality: 't3-content-quality',
    intent_mismatch: 't3-content-intent',
    low_eeat: 't3-content-eeat',
    negative_sentiment: 't3-content-sentiment',
    low_originality: 't3-content-originality',
    likely_ai_generated: 't3-ai-generated',
    ...
    spa_routing_broken: 't1-spa-routing',
    ...
    ssl_grade_low: 't1-ssl-grade',
    ssl_chain_invalid: 't1-ssl-chain',
    cname_chain_long: 't1-dns-cname-chain',
    ...
    table_no_headers: 't2-a11y-table',
    ...
    hydration_mismatch: 't1-js-hydration',
